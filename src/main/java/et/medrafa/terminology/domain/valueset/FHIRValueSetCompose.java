package et.medrafa.terminology.domain.valueset;
...
public class FHIRValueSetCompose {
	...
	public ValueSet.ValueSetComposeComponent toHapi() {
		...
		for (FHIRValueSetCriteria include : orEmpty(getInclude())) {
			hapiCompose.addInclude(include.toHapi());
		}
		for (FHIRValueSetCriteria exclude : orEmpty(getExclude())) {
			hapiCompose.addExclude(exclude.toHapi());
		}
		return hapiCompose;
	}
