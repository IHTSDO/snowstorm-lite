...
import et.medrafa.terminology.snomedimport.ImportService;
import et.medrafa.terminology.syndication.SyndicationClient;
import et.medrafa.terminology.syndication.SyndicationFeed;
import et.medrafa.terminology.syndication.SyndicationFeedEntry;
		...
		try {
			if (Strings.isEmpty(useSyndication)) {
				...
				if (indexDirectory.isDirectory() && files != null && files.length > 0) {
					...
					logger.info("Terminology started. Ready.");
				} else {
					logger.info("Terminology started. Please load a SNOMED CT package.");
				}
			}
